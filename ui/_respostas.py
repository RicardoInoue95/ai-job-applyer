...
def render(com_cabecalho: bool = True) -> None:
    """Desenha a revisão. Chamada pela página própria e, sem cabeçalho, como
    seção de Configurações — memória revisável mora ao lado do ajuste."""
    if com_cabecalho:
        _ui.cabecalho(
    ...
    if _ERRO_BACKEND is not None:
        st.error(f"Banco indisponível: {_ERRO_BACKEND}")
        return
