...
with col1:
    ...
    from jobapplier.applicators.linkedin import has_session as _li_has_session
...
with col2:
    ...
    resume_exists = paths.RESUME_JSON.exists()
        ...
        if rejeitadas_count:
            if st.button(f"🔄 Re-avaliar {rejeitadas_count} rejeitadas com novos critérios", key="reavaliar"):
                with st.spinner("Re-avaliando vagas com threshold 65/45..."):
                    try:
                        from jobapplier.database.connection import get_session as _gs
                        from jobapplier.database.models import Vaga as _V
                        with _gs() as s:
                            s.query(_V).filter(
                                _V.status == "rejeitada",
                                _V.score >= 45,
